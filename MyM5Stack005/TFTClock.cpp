 ...
 *   TFT_Clock.ino
 *   
 * add tft_clock_ymd(), tft_clock_get_current_time(), tft_clock_redraw() by ka's
 * 
 */
...
void tft_clock_analog_setup(void)
{
...
void tft_clock_analog_update(void)
{
  if (targetTime < millis()) {
    if( mm==01 && ss==00 ) // 毎時1分にRTCと同期する
    {
...
/***********************************************************************
  Function : TFT Clock misc. function by ka's
 ***********************************************************************/
void tft_clock_ymd(void)
{
    char ymd[10];
    sprintf(ymd, "%02d-%02d-%02d", y, m, d);
    M5.Lcd.setTextColor(TFT_WHITE, TFT_BLACK);  // Adding a background colour erases previous text automatically
    M5.Lcd.drawCentreString(ymd,120,260,4);
}
